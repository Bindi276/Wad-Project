...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const confirmLink = `http://localhost:5000/api/users/confirm-user-email/${token}`;
      //send email to complete regiseration
      await transport.sendMail({
        from: `SoulAndMind Team 👻 <${process.env.EMAIL_USER}>`,
        to: email,
...
router.get('/confirm-user-email/:token', async (req, res) => {
  try {
    ...
    res
      .status(200)
      .redirect(`http://localhost:3000/user-email-confirmed`);
  } catch (err) {
    console.log(err.message);
    res.status(500).send('Server error');
  }
